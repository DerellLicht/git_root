Fair — I'll stay tight on this one.

WM_NOTIFY basics:

wParam = the control ID that sent it. lParam = a pointer to an NMHDR (or a larger struct that starts with an NMHDR) — always check ((NMHDR*)lParam)->code to know what you're actually looking at before casting further.
